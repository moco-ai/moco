...
セッション: {settings['session_id'] or '(新規)'}"""
        ...
        if text_lower == "/alpaca":
            try:
                with httpx.Client() as http:
                    # moco の alpaca ツールを間接的に呼ぶ代わりに、とりあえず口座情報を取得
                    # (本来は moco API を通じてエージェントに聞くのが理想だが、コマンドとしても実装)
                    # ここでは一旦「Alpaca情報を取得中...」と返し、スレッドで詳細を取得する
                    client.reply_message("🔍 Alpaca 口座情報を取得中...", ev)
                    
                    # 簡易的にセッションを開始してエージェントに「Alpacaの口座要約を教えて」と送る
                    payload = {
                        "message": "Alpacaの口座要約を教えて。数値は分かりやすく整理して。",
                        "session_id": settings["session_id"],
                        "profile": settings["profile"],
                        "provider": settings["provider"],
                        "working_directory": settings["working_dir"]
                    }
                    def fetch_alpaca_info():
                        with httpx.Client(timeout=30.0) as h:
                            r = h.post(MOCO_API_URL, json=payload)
                            if r.status_code == 200:
                                res = r.json().get("response", "取得失敗")
                                client.reply_message(f"📈 [Alpaca]\n{res}", ev)
                            else:
                                client.reply_message("❌ Alpaca情報の取得に失敗しました", ev)
                    
                    threading.Thread(target=fetch_alpaca_info, daemon=True).start()
                return
            except Exception as e:
                client.reply_message(f"⚠️ エラー: {e}", ev)
                return
