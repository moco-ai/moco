...
class AgentRuntime:
    ...
    def _run_gemini_sync(self, user_input: str, history: Optional[List[Any]] = None, session_id: Optional[str] = None) -> str:
        ...
        while True:
            ...
            # イテレーション警告はコメントアウト（トークン上限で管理）
            # remaining = max_iterations - iterations
            # if remaining <= 3 and remaining > 0 and messages:
            #     warning_text = ...
                ...
                if self.stream:
                    ...
                    for chunk in response_stream:
                        ...
                        for part in candidate.content.parts or []:
                            # 思考プロセスの表示（thinking mode: part.thought=True のときテキストが思考内容）
                            if hasattr(part, 'thought') and part.thought and part.text:
                                ...
                                continue  # 思考テキストは通常テキストとして処理しない
                            if part.text:
                                _safe_stream_print(part.text)
                                collected_text += part.text
                                self._partial_response = collected_text  # エラー時の復旧用
                    ...
                    if function_calls:
                        ...
                        # 空の場合は部分応答を返す（Gemini が最終テキストを返さなかった場合の対策）
                        if not collected_text and self._partial_response:
                            return self._partial_response
            ...
            messages.append(content) # アシスタントの応答を履歴に追加
            ...
            # ツール実行結果をメッセージに追加してループ (Geminiに結果を渡す)
            messages.append(types.Content(role="user", parts=tool_responses))
            
            # 80%超過時にコンテキスト圧縮
            usage_ratio = self._accumulated_tokens / MAX_CONTEXT_TOKENS
            if usage_ratio >= CONTEXT_WARNING_THRESHOLD:
                # Gemini メッセージを Dict に変換して圧縮
                dict_messages = _gemini_messages_to_dict(messages)
                compressor = ContextCompressor(
                    max_tokens=int(MAX_CONTEXT_TOKENS * CONTEXT_WARNING_THRESHOLD),
                    preserve_recent=10
                )
                compressed_dict, was_compressed = compressor.compress_if_needed(dict_messages, self.provider)
                if was_compressed:
                    # Dict を Gemini 形式に戻す
                    messages = _dict_to_gemini_messages(compressed_dict)
                    self._accumulated_tokens = compressor.estimate_tokens(compressed_dict)
                    if self.verbose:
                        print(f"\n🗜️ [Context compressed: {self._accumulated_tokens:,} tokens]")
