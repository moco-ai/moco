...
def show_welcome_dashboard(orchestrator, theme_config):
    ...
    # 実際の実装からデータを抽出
    profile = getattr(orchestrator, 'profile', 'default')
    model = getattr(orchestrator, 'model', 'default')
    provider = getattr(orchestrator, 'provider', 'auto')
    work_dir = getattr(orchestrator, 'working_directory', os.getcwd())
    ...
    agent_names = [name for name in agents.keys() if name != "orchestrator"]
    tools_count = len(getattr(orchestrator, 'tool_map', {}))
    skills_count = len(getattr(orchestrator, '_all_skills', {}))
    use_optimizer = getattr(orchestrator, 'use_optimizer', False)
    ...
    runtime_info.add_row(f"[bold {theme_config.thoughts}]Model:[/]", f" {model}")
    runtime_info.add_row(f"[bold {theme_config.thoughts}]Provider:[/]", f" {provider}")
    runtime_info.add_row(f"[bold {theme_config.thoughts}]Optimizer:[/]", f" {'[green]Enabled[/]' if use_optimizer else '[dim]Disabled[/]'}")
    ...
    # 3. Inventory (Agents & Skills)
    inventory_table = Table(box=box.SIMPLE, show_header=True, header_style=f"bold {theme_config.accent}", expand=True)
    inventory_table.add_column("Type", ratio=1)
    inventory_table.add_column("Count / Details", ratio=3)
    ...
    footer = Text.assemble(
        ("Quick Commands: ", "bold"),
        ("/chat ", f"bold {theme_config.warning}"), "Chat ",
        ("/run ", f"bold {theme_config.warning}"), "Task ",
        ("/help ", f"bold {theme_config.warning}"), "Help ",
        ("/exit ", "dim"), "Quit"
    )
    console.print(Panel(footer, style="dim", box=box.HORIZONTALS))
    console.print()
